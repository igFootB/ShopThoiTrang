...
export default function AdminLookbooksPage() {
  ...
  const fetchLookbooks = async () => {
    try {
      setLoading(true);
      const limit = 1000;
      const response = await adminApi.getLookbooks({ page: 0, limit });
      ...
      console.error("Lỗi khi tải danh sách Lookbook:", error);
  ...
  const handleToggleStatus = async (Lookbook: Lookbook) => {
    try {
      const newStatus = Lookbook.trangThai === 1 ? 0 : 1;
      await adminApi.updateLookbook(Lookbook.id, { ...Lookbook, trangThai: newStatus });
      toast.success(`Đã ${newStatus === 1 ? "hiện" : "ẩn"} Lookbook`);
      fetchLookbooks();
    } catch (error) {
  ...
  const handleHardDelete = async (id: number) => {
    if (confirm("Lưu ý: Hành động này sẽ xóa vĩnh viễn Lookbook khỏi CSDL. Bạn có chắc chắn muốn xóa?")) {
      try {
        await adminApi.deleteLookbook(id);
        toast.success("Đã xóa vĩnh viễn Lookbook");
        fetchLookbooks();
      } catch (error) {
        toast.error("Không thể xóa Lookbook này");
      }
    }
  };
  ...
  const openEditModal = (Lookbook: Lookbook) => {
    setModalMode("edit");
    setSelectedLookbook(Lookbook);
    setFormData({
      tieuDe: Lookbook.tieuDe || "",
      moTa: Lookbook.moTa || "",
      hinhAnh: Lookbook.hinhAnh || "",
      duongDan: Lookbook.duongDan || "",
      trangThai: Lookbook.trangThai,
    });
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.hinhAnh) {
        toast.error("Hình ảnh Lookbook là bắt buộc");
        return;
    }
    ...
    try {
      if (modalMode === "add") {
        ...
        toast.success("Thêm Lookbook mới thành công");
      } else if (modalMode === "edit" && selectedLookbook) {
        await adminApi.updateLookbook(selectedLookbook.id, formData);
        toast.success("Cập nhật Lookbook thành công");
      }
      closeModal();
      fetchLookbooks();
    } catch (error) {
      console.error(error);
      toast.error(`Có lỗi xảy ra khi ${modalMode === "add" ? "thêm" : "cập nhật"} Lookbook`);
    }
  };

  const filteredLookbooks = Lookbooks.filter(Lookbook =>
    (Lookbook.tieuDe?.toLowerCase() || "").includes(searchQuery.toLowerCase())
  );
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Quản lý Lookbook trang chủ</h1>
          <p className="text-sm text-gray-500 mt-1">Quản lý các Lookbook lớn hiển thị trên Slide trang chủ</p>
        </div>
        <button
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                ...
                <tr>
                  <td colSpan={5} className="py-12 text-center text-gray-500">
                    Không tìm thấy Lookbook nào.
                  </td>
                </tr>
              ) : (
                filteredLookbooks.map((Lookbook) => (
                  <tr key={Lookbook.id} className="hover:bg-blue-50/30 transition-colors group">
                    <td className="py-4 px-6 text-sm font-medium text-gray-900">#{Lookbook.id}</td>
                    <td className="py-4 px-6">
                      <div className="relative w-32 h-16 rounded-lg overflow-hidden border border-gray-200 bg-gray-50">
                        {Lookbook.hinhAnh ? (
                          <Image src={Lookbook.hinhAnh} alt={Lookbook.tieuDe || "Lookbook"} fill className="object-cover" unoptimized />
                        ) : (
                    ...
                    <td className="py-4 px-6">
                      <div className="flex flex-col">
                        <span className="text-sm font-semibold text-gray-900">{Lookbook.tieuDe || "Không có tiêu đề"}</span>
                        <span className="text-xs font-medium text-gray-400 mt-0.5 truncate max-w-[200px]">{Lookbook.duongDan || "Không URL liên kết"}</span>
                      </div>
                    </td>
                    <td className="py-4 px-6 text-center">
                      <span
                        className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium border ${Lookbook.trangThai === 1
                          ? "bg-green-50 text-green-700 border-green-200/50"
                          : "bg-orange-50 text-orange-700 border-orange-200/50"
                          }`}
                      >
                        {Lookbook.trangThai === 1 ? "Hoạt động" : "Đã ẩn"}
                      </span>
                    </td>
                    <td className="py-4 px-6 text-right">
                      <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button
                          onClick={() => handleToggleStatus(Lookbook)}
                          className={`p-2 rounded-lg transition-colors cursor-pointer ${Lookbook.trangThai === 1 ? 'text-orange-500 hover:bg-orange-50' : 'text-green-600 hover:bg-green-50'}`}
                          title={Lookbook.trangThai === 1 ? "Ẩn Lookbook" : "Hiện Lookbook"}
                        >
                          {Lookbook.trangThai === 1 ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                        </button>
                        <button
                          onClick={() => handleHardDelete(Lookbook.id)}
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h3 className="text-lg font-bold text-gray-900">
                {modalMode === "add" ? "Thêm Lookbook mới" : "Sửa Lookbook"}
              </h3>
              <button
                type="button"
                onClick={closeModal}
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                    Hình ảnh Lookbook <span className="text-red-500">*</span>
                </label>
              ...
              <div className="flex items-center justify-end gap-3 mt-8">
                ...
                <button
                  type="submit"
                  disabled={uploadingImage}
                  className="px-5 py-2.5 text-sm font-medium text-white bg-blue-600 rounded-xl hover:bg-blue-700 transition-colors shadow-sm cursor-pointer disabled:opacity-50"
                >
